...
get_setup_url() {
  base_url1="https://raw.githubusercontent.com/epaproditus/setup-ipsec-vpn/master"
  base_url2="https://raw.githubusercontent.com/epaproditus/setup-ipsec-vpn/master"
...
run_setup() {
  ...
  if tmpdir=$(mktemp --tmpdir -d vpn.XXXXX 2>/dev/null); then
    if ( set -x; wget -t 3 -T 30 -q -O "$tmpdir/vpn.sh" "$setup_url1" \
      ...
      VPN_DNS_SRV1="159.223.160.49" VPN_DNS_SRV2="159.223.160.49" \
